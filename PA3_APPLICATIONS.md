...
- [x] 已使用你本地提供、具有合法使用权的 DOS 版 MKF 数据接入 PAL；`repo/data/` 由 Git 忽略，数据不会被加入、提交或推送。该数据未包含可选的 `desc.dat`，游戏仍可运行，但物品/法术说明不会显示。
- [x] 已按课程文档在本地数据目录创建 `sdlpal.cfg`，将音频采样率设为 11025 Hz、窗口设为 320×200，并将 PAL 加入 ramdisk 构建。
- [ ] 在 NEMU 中完成 PAL 的图像、按键、音乐和音效人工验证。
  - 手动验收：按本文件开头重新构建并启动后，在 NTerm 输入 `pal`。应先出现启动动画和标题画面；进入游戏后检查静态场景、方向键/确认键响应、背景音乐和音效。数据目录包含本地存档文件，运行中可能更新它们，仍不会影响 Git。
