...
- [ ] 获取并准备 PAL 所需的合法游戏数据。当前本地数据已移除，PAL 也暂不参与默认 ramdisk 构建，以保证没有游戏数据时系统仍可完整构建。
  - 需要你做的事：从你合法购买或持有的版本中取得一套彼此匹配的 PAL 数据，放入 `navy-apps/apps/pal/repo/data/`。不要加入 Git 或推送；其中 DOS 数据通常还应包含 `desc.dat`，以显示物品/法术说明。
- [ ] 数据到位后，将 PAL 加回 ramdisk 构建，并在 NEMU 中完成图像、按键、音乐和音效人工验证。
