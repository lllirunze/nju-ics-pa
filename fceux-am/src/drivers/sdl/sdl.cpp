...
const char *FCEUI_DefaultROM(void);
 ...
 */
#ifdef __NO_FILE_SYSTEM__
static int fceux_main(const char *romname)
#else
...
int main(int argc, char *argv[]) {
  (void)argc;
  (void)argv;
  const char *romname = FCEUI_DefaultROM();
  if (romname == NULL) {
    printf("No NES ROM is installed. Put a legal .nes file in nes/rom/.\n");
    return 1;
  }
  return fceux_main(romname);
}
#endif
