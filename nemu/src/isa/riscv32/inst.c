...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) =
      (word_t)(((uint64_t)((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2)) >> 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) =
      (word_t)(((uint64_t)((int64_t)(sword_t)src1 * (int64_t)src2)) >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) =
      (word_t)(((uint64_t)src1 * (uint64_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = src2 == 0 ? -1 :
      (src1 == 0x80000000u && src2 == 0xffffffffu ? src1 : (sword_t)src1 / (sword_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src2 == 0 ? -1 : src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = src2 == 0 ? src1 :
      (src1 == 0x80000000u && src2 == 0xffffffffu ? 0 : (sword_t)src1 % (sword_t)src2));
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = src2 == 0 ? src1 : src1 % src2);
  INSTPAT("??????? ????? ????? ????? ????? 01101 11", lui    , U, R(rd) = imm);
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 00000 00011 11", fence  , N, );
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I,
      ...
      IFDEF(CONFIG_FTRACE, if (rd == 0 && imm == 0 && (BITS(s->isa.inst, 19, 15) == 1 || BITS(s->isa.inst, 19, 15) == 5)) ftrace_ret(s->pc));
      IFDEF(CONFIG_FTRACE, if (rd == 1 || rd == 5) ftrace_call(s->pc, s->dnpc)));
