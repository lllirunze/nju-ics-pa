...
again:
  ...
  INSTPAT("0000 0101", add,       I2a,  0, word_t lhs = Rr(rd, w), result = lhs + imm; Rw(rd, w, result); set_add_flags(lhs, imm, result, w));
  ...
  INSTPAT("0001 0101", adc,       I2a,  0, word_t rhs = imm + (cpu.eflags & EFLAGS_CF ? 1 : 0), lhs = Rr(rd, w), result = lhs + rhs; Rw(rd, w, result); set_add_flags(lhs, rhs, result, w));
  ...
  INSTPAT("0010 1101", sub,       I2a,  0, word_t lhs = Rr(rd, w), result = lhs - imm; Rw(rd, w, result); set_sub_flags(lhs, imm, result, w));
  ...
  INSTPAT("0011 1100", cmp,       I2a,  1, word_t lhs = Rr(rd, w); set_sub_flags(lhs, imm, lhs - imm, w));
  INSTPAT("0011 1101", cmp,       I2a,  0, word_t lhs = Rr(rd, w); set_sub_flags(lhs, imm, lhs - imm, w));
  ...
  INSTPAT("0010 0101", and,       I2a,  0, word_t result = Rr(rd, w) & imm; Rw(rd, w, result); set_zs(result, w); cpu.eflags &= ~(EFLAGS_CF | EFLAGS_OF));
